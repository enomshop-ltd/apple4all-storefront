  ...
  if (orders.length === 0) {
    return (
      <div class="text-center py-12 border border-gray-200 rounded-xl bg-gray-50">
        ...
        <a href="/" f-client-nav class="inline-flex items-center justify-center px-6 py-2.5 bg-gray-900 text-white font-medium rounded-lg hover:bg-gray-800 transition-colors">
          Continue shopping
        </a>
      </div>
    );
  ...
  return (
    <div class="space-y-6">
      {orders.map((order) => {
        ...
        return (
        <div key={order.id} class="border border-gray-200 rounded-xl overflow-hidden">
          <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center gap-6">
              <div>
                <p class="text-xs text-gray-500 uppercase tracking-wider font-medium mb-1">Order placed</p>
            ...
            <div class="flex items-center gap-4">
              ...
              <span class="text-sm text-gray-500">Order #{order.display_id || order.id.split('_')[1]}</span>
            </div>
          </div>
          ...
          <div class="p-6">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-4">
                ...
                <div>
                  <h4 class="font-medium text-gray-900">{order.items?.[0]?.title}</h4>
                  {order.items && order.items.length > 1 && (
                    <p class="text-sm text-gray-500">and {order.items.length - 1} more items</p>
                  )}
                </div>
              </div>
              
              <button class="flex items-center gap-2 text-sm font-medium text-blue-600 hover:text-blue-800 transition-colors">
                View details
