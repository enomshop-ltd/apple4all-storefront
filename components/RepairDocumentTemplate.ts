export function RepairDocumentTemplate({ ticket, type, qrCodeUrl, logoUrl }: { ticket: any, type: string, qrCodeUrl: string, logoUrl: string }): string {
  ...
  if (ticket.parts && ticket.parts.length > 0) {
    partsRowsHtml = ticket.parts.map((p: any, idx: number) => `
      <tr class="border-b border-[#e5e7eb]">
        ...
        <td class="py-3.5 px-3">${p.title} ${p.product?.title ? `(${p.product.title})` : ""}</td>
        <td class="py-3.5 px-3 text-center">1.00</td>
        <td class="py-3.5 px-3 text-right">${formatAmount(p.price || p.estimated_price || 0)}</td>
        <td class="py-3.5 px-3 text-right">${formatAmount(p.price || p.estimated_price || 0)}</td>
      </tr>
    `).join("");
  } else {
    partsRowsHtml = `
      <tr class="border-b border-[#e5e7eb]">
        ...
        <td class="py-3.5 px-3">Standard Repair Service ${type === "quote" ? "(Estimate)" : ""}</td>
        <td class="py-3.5 px-3 text-center">1.00</td>
      ...
      <table class="w-full text-[12px] border border-[#e5e7eb] rounded-lg overflow-hidden">
        <tbody class="divide-y divide-[#e5e7eb] text-[#1f2937]">
          <tr>
            ...
            <td class="py-3 px-4">${ticket.physical_condition || "Not specified during intake."}</td>
          </tr>
          <tr>
      ...
      <div class="w-[320px]">
        ...
        ${isReceipt || type === 'invoice' ? `
          <div class="flex justify-between py-2 text-[12px]">
            ...
            <span class="text-[#ef4444] text-right w-32">(-) ${ticket.status === 'completed' ? total : '0.00'}</span>
          </div>
        ` : ''}
        ...
        <div class="pb-1">
          ...
          ${ticket.device?.brand ? `
            <div class="mt-2 text-[#4b5563] text-[11px] max-w-[200px]">
               <p><span class="font-semibold text-[#1f2937]">Device:</span> ${ticket.device.brand} ${ticket.device.model_name || ''}</p>
               <p><span class="font-semibold text-[#1f2937]">Serial:</span> ${ticket.device.serial_number || "N/A"}</p>
            </div>
          ` : ''}
        </div>
        <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-[12px] text-right w-64 pb-1">
          <div class="text-[#4b5563]">${type === 'quote' ? 'Estimate Date' : 'Invoice Date'} :</div>
          <div class="text-[#111827]">${date}</div>
